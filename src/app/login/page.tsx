...
function LoginForm() {
  ...
  const errorMessages: Record<string, string> = {
    ...
    AccessDenied: "Access denied. Only @level.agency accounts are allowed.",
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-start bg-black text-white px-4 py-12 overflow-y-auto">
      ...
      {/* Error Display */}
      {errorMessage && (
        <div className="w-full max-w-md mb-4 p-4 rounded-xl border border-[#FD6EF8] bg-[#FD6EF8]/10 text-center">
          <p className="text-[#FD6EF8] text-sm font-semibold" style={{ fontFamily: "'DM Sans', sans-serif" }}>
            {errorMessage}
          </p>
        </div>
      )}
      ...
      <div className="w-full max-w-md bg-[#111] border border-[#2A2A2A] rounded-2xl p-8 text-center mb-6">
        ...
        <button
          ...
          disabled={loading}
          className={`w-full py-3 px-4 font-bold rounded-lg transition-colors duration-200 flex items-center justify-center gap-3 ${
            loading
              ? "bg-[#86D5F4]/50 cursor-not-allowed text-black/50"
              : "bg-[#86D5F4] hover:bg-[#6bc5e8] text-black cursor-pointer"
          }`}
